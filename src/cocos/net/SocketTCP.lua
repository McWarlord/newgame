...
local STATUS_SHOTDOWN = "Cannot send after socket shutdown"

local scheduler = require("cocos.net.scheduler")
...
function SocketTCP:ctor(__host, __port, __retryConnectWhenFailure)
	--cc:addComponent("components.behavior.EventProtocol"):exportMethods()
...
function SocketTCP:_onDisconnect()
    ...
    --print(self.name .. "SocketTCP:_onDisconnect --------------------------------- ");
	self.isConnected = false
	self:dispatchEvent({name=SocketTCP.EVENT_CLOSED})
	self:_reconnect();
end
...
--			print("body:", __body, "__status:", __status, "__partial:", __partial)
    	    if __status == STATUS_CLOSED or __status == STATUS_NOT_CONNECTED or __status == STATUS_SHOTDOWN then
		    	self:close()
...
function SocketTCP:addEventListener(eventName, listener, tag)
...
function SocketTCP:dispatchEvent(event)
...
function SocketTCP:removeEventListener(handleToRemove, key1, key2)
    if key2 then
...
function SocketTCP:removeEventListenersByTag(tagToRemove)
    for eventName, listenersForEvent in pairs(self.listeners_) do
...
function SocketTCP:removeEventListenersByEvent(eventName)
...
function SocketTCP:removeAllEventListenersForEvent(eventName)
...
function SocketTCP:removeAllEventListeners()
    self.listeners_ = {}
    if DEBUG > 1 then
...
function SocketTCP:hasEventListener(eventName)
...
function SocketTCP:dumpAllEventListeners()
...
function SocketTCP:onBind_()
end

function SocketTCP:onUnbind_()
end
